...
\begin{definition}
    Let $F$ be a free group of rank $m$. Given a surjective homomorphism $\beta \colon F \rightarrow L/M$, we define the set $\mathbf{\mathscr{R}_\beta}$ as the set of normal subgroups $N$ of $F$ arising as kernels of those homomorphisms of $F$ onto $L$ which composed with the natural projection $\pi_L \colon L \rightarrow L/M$ yield $\beta$.
\end{definition}
...
\begin{proof}
    ...
    Now given $N \in \mathscr{R}_\beta$, let us choose a function $\gamma_N$ such as in the proof of Theorem \ref{PHallT}. Let us also consider the subsequence of $N_1, ..., N_r$:
    $$N_{i_1} = N_1, ..., N_{i_q}$$
    ...
    And we will prove by induction that for $1 \le s \le q$ the function
    \begin{align*}
        \Psi_s \colon &F \rightarrow L_s \\
                    &x \mapsto (\gamma_{N_1}(x), ... , \gamma_{N_s}(x))
    \end{align*}
    is surjective homomorphism with kernel $\bigcap_{i=1}^q N_i$. After this is proved we can easily conclude that $F/(\bigcap_{N \in \mathscr{R}_\beta}N) \cong L_q$ due to $\bigcap_{N \in \mathscr{R}_\beta}N = \bigcap_{i=1}^q N_i$ and the First isomorphism Theorem.
    ...
    We also easily obtain that $\ker \Psi_{s+1} = \bigcap_{i=1}^{q} N_i =\bigcap_{N \in \mathscr{R}_\beta}N$ since for any $1 \le i \le k$, $\ker \gamma_{N_i} = N_i$.

...
\begin{proof}
    ...
    \begin{align*}
        ...
        \iff &y^{-1}kxk^{-1} = \phi_r(y)k\phi_r(x)^{-1}k^{-1} \in N_1 \cap N_r = 1\\
    ...
    Now by Theorem \ref{hommnsub}, $N_1 = \Psi_1(N_1)$ is a minimal normal subgroup of $L$. We claim it is the only minimal normal subgroup of $L$.
    ...
    Now it is not difficult to see that $\Psi_i^{-1}(A) = AC_i$ and that these groups are normal subgroups of $H$. Furthermore $C_1\ldots C_q \cap A \subseteq C_1\ldots C_q \cap K \subseteq \soc{H} \cap K = 1$ and thus by Theorem
    ...
    Now we claim that $C_L(N_1) = N_1$. To obtain a contradiction let us suppose that there exists elements $k \in K$ and $n_1 \in N_1$ such that $kn_1 \notin N_1$ (that is $k \ne 1$) and $kn_1 \in C_L(N_1)$. Since $N_1$ is abelian, $n_1^{-1} \in C_L(N_1)$ and thus $k = (kn_1)n_1^{-1} \in C_L(N_1)$. What we proved until now was that $C_L(N_1) \cap K \ne 1$. Now since $C_L(N_1)$ is normal in $L$, $K \le N_L(C_L(N_1) \cap K)$ and since $N_1$ is abelian, $N_1 \le N_L(C_L(N_1) \cap K)$. Thus $L = KN_1 \le N_L(C_L(N_1) \cap K)$. What we conclude is that $C_L(N_1) \cap K$ is a non-trivial normal subgroup of $L$ fully contained in $K$, but this contradicts the fact that $\core[L]{K} = 1$.
...
\begin{align*}
    ...
    &= \phi_r^{-1}(\phi_r(x^k)^{\phi_r(m_r)}) \\
    &= \phi_r^{-1}(\phi_r(x^k))^{\phi_r^{-1}(\phi_r(m_r))}\\
